...
| 2a — circuit purpose | ✅ **SOLVED (2026-08-15)** — the chip is a **2-star Star Battle checker on an 11×11 grid**; region map recovered from silicon; unique solution == the key; rules decoded flag-by-flag (`recon/opam/README.md`) |
...
4. **Confirm on the reference model** (`debug_S1/full.v`): replay the key through the
   reference sky130 sim. `success` goes high at cycle 125 and stays high; `O` spells
...
| decoder reports `success=0` even when it fired | early decoder read the wrong cycle; `success` in fact **self-holds** once fired | read it after the last board bit (cycle ≥125) |
...
- The display holds five messages: `(* TWO STARS *)`, `TWO NOT TOUCH`, `TRY AGAIN`, `EMPTY SKY`, `BIG BANG`.
- The chip *is* a Star Battle puzzle; `(* TWO STARS *)` names the puzzle type; 22 ones = 2 stars × 11.
...
**Summary (final, 2026-08-15):** the "scrambled hash lock" reading is **superseded**. Enumerating the
22 accept-check flop pairs shows each is an *independent* 2-bit saturating counter stepped at
counter-selected input positions: 11 count the columns of an 11×11 grid, 11 count irregular
regions; two sticky flags check rows and adjacency. **The chip is a two-star Star Battle checker.**
The region map was read out of the silicon and solved independently → **one and only one
solution, == the key** (31,197,434 boards satisfy rows/cols/no-touch alone; the regions leave one).
The board is 121 cells; the 122nd input bit is a don't-care. The 57-bit "target constant" is simply
"every counter reads 2, no flag set". Proof status: datapath ≡ netlist formally (comb. SAT miter,
`recon/opam/comb_miter.log`); assembled RTL ≡ netlist by simulation only (sequential SEC did not
complete — async-FF error in `recon/build_miter.log`); key confirmed on sky130 reference cells
(`recon/opam/tb_ref_sky130.v`). All in `recon/opam/`.

...
NETLIST  = os.path.join(HERE, "..", "extracted_puzzle.v")
...
META     = os.path.join(HERE, "..", "linearity", "flop_meta.json")
...
rows, equations = [], []
for iname in sorted(flops, key=lambda s: int(s.split("__")[1])):
    ctype, pins = flops[iname]
    dnet = pins["D"]
    sup = support(dnet)
    try:
        f = anf(dnet)
        deg   = max((len(m) for m in f), default=0)
        nmon  = len(f)
        lin   = sorted(v for m in f if len(m) == 1 for v in m)
        nlmon = sorted(["&".join(sorted(m)) for m in f if len(m) >= 2])
        shift = "SHIFT" if nmon == 1 and deg == 1 else ""
        equations.append(f"{iname}' = " + " ^ ".join(
            (["1"] if frozenset() in f else []) + lin + nlmon))
    except Blowup:
        deg, nmon, shift = ">?", f">{CAP}", "BLOWUP(comparator-like)"
        equations.append(f"{iname}' = [ANF > {CAP} monomials; support={len(sup)}: "
                         + ",".join(sorted(sup)) + "]")
    cone, absI = (meta.get(iname, ["?", "?", "?"])[1:3])
    rows.append((iname, ctype, cone, absI, len(sup), deg, nmon, shift))

with open(os.path.join(HERE, "anf_report.md"), "w") as r:
    r.write("| flop | type | cone | absI | support | degree | #monomials | note |\n|---|---|---|---|---|---|---|---|\n")
    for t in rows: r.write("| " + " | ".join(str(x) for x in t) + " |\n")
open(os.path.join(HERE, "anf_equations.txt"), "w").write("\n".join(equations))
print(f"{len(rows)} flops -> anf_report.md, anf_equations.txt")
degs = [t[5] for t in rows if isinstance(t[5], int)]
print("degree histogram:", {d: degs.count(d) for d in sorted(set(degs))})
print("pure shift bits :", sum(1 for t in rows if t[7] == "SHIFT"))

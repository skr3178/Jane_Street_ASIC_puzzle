...
def emit_verilog(ly, top, nl, tc, out, pin_dirs):
    ...
    # PASS 1: gather each logic cell's named pins (pin, net, is_output) and any
    # anonymous-pin nets. Learn which nets are driven by a cell output pin.
    raw = []       # (cell, inst, [(pin, net, is_output)], [anon_net...])
    driven_by_output = set()
    dangling = 0
    for sc in tc.each_subcircuit():
        cir = sc.circuit_ref()
        if not is_logic_cell(cir.name):
            continue
        named, anon = [], []
        for i in range(cir.pin_count()):
            ...
            if not pn:
                anon.append(nn)
                continue
            is_out = pin_dirs.get(cir.name, {}).get(pn) == "OUTPUT"
            named.append((pn, nn, is_out))
            if is_out:
                driven_by_output.add(nn)
        raw.append((cir.name, sc.expanded_name(), named, anon))

    # PASS 1.5: repair internal-li1 feedthroughs. When a net makes an anonymous
    # (non-pin) touch on a cell AND one of that cell's named input pins is left
    # undriven, they are the same physical net bridged through the cell's own
    # local interconnect -- which the black-box trace cannot see but a full
    # device extraction (Magic) does. Merge the undriven pin net into the
    # (driven) touch net. Conservative: only when exactly one of each on a cell.
    def is_boundary(n):
        return n in port_names or n in ("VPWR", "VGND")
    alias = {}
    merges = []
    for cell, inst, named, anon in raw:
        if len(anon) != 1:
            continue
        undriven_ins = [n for (p, n, o) in named
                        if not o and n not in driven_by_output and not is_boundary(n)]
        if len(undriven_ins) == 1 and undriven_ins[0] != anon[0]:
            alias[undriven_ins[0]] = anon[0]
            merges.append((cell, inst, undriven_ins[0], anon[0]))
    merge_targets = {ann for (_, _, _, ann) in merges}
    ...
    # PASS 2: build final instances with merged net names.
    insts = []
    used_nets = set()
    driven_by_output = {resolve(n) for n in driven_by_output}
    unnamed = []   # anon touches that were NOT resolved into a pin (left as note)
    for cell, inst, named, anon in raw:
        conns = []
        for pn, nn, is_out in named:
            rn = resolve(nn)
            used_nets.add(rn)
            conns.append((pn, rn))
        for nn in anon:
            # an anon touch is redundant once its cell's undriven pin was merged
            # onto it; only warn about touches that no merge consumed.
            if nn not in merge_targets:
                unnamed.append((cell, inst, nn))
    ...
    return {"instances": len(insts), "inputs": len(inputs),
            ...
            "unnamed_pins": unnamed, "merges": merges, "port_bases": portlist}
...
def main():
    ...
    leaked = sorted({sc.circuit_ref().name for sc in tc.each_subcircuit()
                     if not is_logic_cell(sc.circuit_ref().name)})
    ...
    for cell, inst, undn, ann in st.get('merges', []):
        e.write(f"[extract] merged net {undn} -> {ann}: undriven input pin of "
                f"{cell} ({inst}) bridged to an anonymous touch on the same cell "
                f"(internal-li1 feedthrough, confirmed vs Magic device extraction)\n")
    for cell, inst, net in st.get('unnamed_pins', []):
        e.write(f"[extract] WARNING: net {net} touches an unlabeled internal "
                f"node of {cell} ({inst}) -- skipped as a non-pin connection, and "
                f"no undriven pin on that cell to merge it with; revisit at device "
                f"level if analysis stalls\n")

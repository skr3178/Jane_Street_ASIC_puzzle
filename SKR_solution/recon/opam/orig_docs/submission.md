...
- [ ] Recover netlist from `puzzle.gds`
- [ ] Determine circuit purpose
- [ ] Find input sequence that drives `success` high
- [ ] Simulate output generator to read the final string
- [ ] Note any Easter eggs
- [ ] Write up approach and submit

Known so far: example stimulus in `example_inputs.vcd` is 242 bits on `I` (fails); `O[7:0]` is ASCII
